...
class ConversationalAIViewModel: ObservableObject {
    ...
    func endConversation() {
        ...
        Task {
            ...
            DispatchQueue.main.async {
                self.conversation?.stopRecording()
                self.conversation?.endSession()
                self.conversation = nil
                self.status = .disconnected
                
                // need to reset AV playmode to fix error (play and record -> playback)
                do {
                    let audioSession = AVAudioSession.sharedInstance()
                    
                    // Step 1: Deactivate the session to release the previous settings
                    try audioSession.setActive(false, options: .notifyOthersOnDeactivation)
                    
                    // Step 2: Set the category to Playback (removes recording capabilities)
                    try audioSession.setCategory(.playback, mode: .default, options: [])
                    
                    // Step 3: Reactivate the session with the new settings
                    try audioSession.setActive(true, options: [])
                    
                    print("Successfully reset to playback mode")
                } catch {
